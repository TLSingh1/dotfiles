...
Item {
    ...
    function updateHyprland(colors) {
        ...
        // Create hyprland color config with proper syntax
        const hyprColors = `# Dynamic theme colors from wallpaper
# Generated by Quickshell Cyberpunk Theme

general {
    col.active_border = rgb(${colors.neonPrimary.slice(1)}) rgb(${colors.neonSecondary.slice(1)}) 45deg
    col.inactive_border = rgb(${colors.surface.slice(1)})
}

decoration {
    col.shadow = rgba(${colors.primary.slice(1)}55)
}

group {
    col.border_active = rgb(${colors.neonPrimary.slice(1)})
    col.border_inactive = rgb(${colors.surface.slice(1)})
}

misc {
    background_color = rgb(${colors.background.slice(1)})
}`
        
        // Write to config directory and reload
        hyprlandUpdater.command = ["bash", "-c", `mkdir -p ~/.config/hypr && echo '${hyprColors}' > ~/.config/hypr/theme.conf && hyprctl reload`]
